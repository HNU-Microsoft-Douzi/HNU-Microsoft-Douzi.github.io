...
`$ ls [abcd]`列出所有文件名以a,b,c,d字母开头的文件。

`$ls[!c-t]* | pg`列出所有除去文件名开头是c 到 t 的文件。结果会很长。
...
`ps -ef`列出系统所有用户进程的详细信息。

`ps aux`列出系统所有进程更多的详细信息。
...
`/etc/init`由内核启动，没有父进程，它是其它进程的父进程。

`ksh、bash`都是用来创建终端的，各个终端的功能都相差不多，有一些细微的差异。

`ksh、bash`创建的子进程不能拷贝父进程的资源，父进程创建的局部变量无法在子进程中使用。

**创建全局变量**

`export x`将x设置为全局变量，却没有办法让父子进程的变量指向同一个内存空间，只是引用的关系而已。
...
$?是预定义的shell环境变来那个，储存上一跳命令或脚本的返回码。

**exit**

`exit`用来退出脚本或当前shell并设置返回值。
...
> echo 是shell内部一个函数，调用时不会产生新的子进程。
...
如果我们远程telnet到主机或使用xterm时不也需要一个终端交互吗?这就是虚拟终端pty(pseudo-tty)
...
前台执行会占用当前shell的命令行，使得该条元素不执行完就无法执行下一条命令。

后台执行将命令运行在后台并不在shell中进行显示，可以使得user继续执行下一条命令。

当我们需要将一个命令转为后台执行时，只需要在命令最后加 `&`。

当父进程异常退出时，子进程就会被接管，此时显示子进程的接管态Z。

**终止进程：**

<ctrl + c>终止当前`shell`的前台进程。

`kill`从其他控制台终止当前控制台的进程。

**终止后台进程:**

kill 后台进程(只能用此命令来终止后台进程。)

kill 204 发送终止信号（通知程序正常结束）

kill -9 206 发送杀死信号（不通知程序直接终止或程序不响应直接终止）

nohup命令将组织因为用户注销而导致进程中指直到该进程正常结束，此命令将忽视信号1和3（若用户不重定向输出，则nohup将重定向输出到nohup.out文件中）

**守护进程(daemon)**

是一个永不结束的进程，在系统启动时执行，在系统结束时结束，用来控制系统资源，提供相关服务，是系统默认启动的进程。